...
use authentication::{SignInRequest, SignUpRequest, SignOutRequest};
...
pub mod authentication { tonic::include_proto!("authentication"); }
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // https://stackoverflow.com/questions/5    9179831/docker-app-server-ip-address-127-0-0-1-difference-of-0-0-0-0-ip
    let auth_hostname = env::var("AUTH_SERVICE_HOST_NAME").unwrap_or("[::0]".to_owned());

    // Establish connection when auth service
    let mut client = AuthClient::connect(format!("https://{}:50051", auth_hostname)).await?;
