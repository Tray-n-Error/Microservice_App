...
impl Users for UsersImpl {
    fn create_user(&mut self, username: String, password: String) -> Result<(), String> {
        if self.username_to_user.get(&username).is_some() {
            return Err(format!("{} exists already", username))
        }
        ...
        self.uuid_to_user.insert(user.clone().user_uuid, user.clone());
        self.username_to_user.insert(user.clone().username, user.clone());

        Ok(())
    }
    fn get_user_uuid(&self, username: String, password: String) -> Option<String> {
        let user: &User = &self.username_to_user.get(&username)?.clone();
        ...
        if result.is_ok() {
            return Some(user.clone().user_uuid)
        }
        None
    }
    fn delete_user(&mut self, user_uuid: String) {
