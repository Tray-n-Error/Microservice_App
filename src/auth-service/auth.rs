...
use authentication::{ SignInRequest, SignInResponse, SignOutRequest, SignOutResponse, SignUpRequest,
...
impl Auth for AuthService {
    async fn sign_up(&self, request: Request<SignUpRequest>) -> Result<Response<SignUpResponse>, Status> {
        ...
        match result {
            Ok(_) => Ok(Response::new(SignUpResponse { status_code: 1 })),
            //V V V V V V Something with tonic::Code?!? V V V V V V
            Err(_) => Ok(Response::new(SignUpResponse { status_code: 0 }))
        }
    }
    async fn sign_in(&self, request: Request<SignInRequest>) -> Result<Response<SignInResponse>, Status> {
        ...
        let user_uuid: String = result.unwrap().to_string();
    ...
    async fn sign_out(&self, request: Request<SignOutRequest>) -> Result<Response<SignOutResponse>, Status> {
        ...
        let reply: SignOutResponse = SignOutResponse { status_code: 1 };
